...
void traceRayFunction(float x, float y)
{
	...
	float tmin = 0.1f;
	RayTracer rt(sp, bounce_limit, cut_off_ray_weight, cast_shadow, Vec3f(nx, ny, nz), grid);
...
int main(int argc, char*argv[])
{
	...
	RayTracer raytracer = RayTracer(sp, bounce_limit, cut_off_ray_weight, cast_shadow, Vec3f(nx, ny, nz), grid);
	g = raytracer.getGrid();
	...
	if (gui){
		GLCanvas canvas;
		canvas.initialize(sp, RenderScene, traceRayFunction, g, visualize_grid);
	}
